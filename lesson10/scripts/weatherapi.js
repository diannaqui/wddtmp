...
if (weather) {
  ...
  function  displayResults(weatherData) {
    ...
    currentTemp.innerHTML = `The current temperature in Fairbanks, Alaska is <strong>${tF}</strong> °F`;
    ...
    for (let i = 0; i < weatherData.weather.length; i++) {
      ...
      const iconsrc = `https://openweathermap.org/img/w/${weatherData.weather[i].icon}.png`;
